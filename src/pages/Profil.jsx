...
function ProfilContent() {
  ...
  const handlePwd = async () => {
    if (newPwd.length < 6) {
      setPwdMsg('Minimum 6 caractères');
      return;
    }
    try {
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ margin: '14px 16px 0', display: 'flex', gap: 8 }}>
            <button
              ...
              style={{
                ...
                padding: '10px 12px',
                borderRadius: 'var(--Rs)',
                border: `1.5px solid ${tab === 'infos' ? 'var(--g)' : 'var(--bd)'}`,
                background: tab === 'infos' ? 'rgba(155,226,45,.1)' : 'transparent',
                color: tab === 'infos' ? 'var(--g)' : 'var(--tx2)',
                fontSize: 13,
                fontWeight: 600,
                cursor: 'pointer',
                transition: 'all .15s',
            ...
            <button
              ...
              style={{
                ...
                padding: '10px 12px',
                borderRadius: 'var(--Rs)',
                border: `1.5px solid ${tab === 'stats' ? 'var(--g)' : 'var(--bd)'}`,
                background: tab === 'stats' ? 'rgba(155,226,45,.1)' : 'transparent',
                color: tab === 'stats' ? 'var(--g)' : 'var(--tx2)',
                fontSize: 13,
                fontWeight: 600,
                cursor: 'pointer',
                transition: 'all .15s',
          ...
          <div
            style={{
              margin: '14px 16px',
              background: 'linear-gradient(135deg, rgba(17,31,23,.96), rgba(5,12,8,.98))',
          ...
          >
            <div
              style={{
                ...
                height: 120,
                background:
                  'radial-gradient(circle at top right, rgba(155,226,45,.12), transparent)',
                pointerEvents: 'none',
              }}
            />
              ...
              <div>
                <div
                  style={{
                    ...
                    color: 'var(--tx)',
                    textShadow: '0 0 12px rgba(155,226,45,.15)',
                  }}
                >
                  {profil?.nom}
                </div>
                ...
                {profil?.role === 'admin' && (
                  <span className="pill pill-g" style={{ marginTop: 4, display: 'inline-flex' }}>
                    Admin
                  </span>
                )}
              </div>
          ...
          >
            {[
              ...
              {
                val: `${Math.round(((stats?.gainsTotal || 0) - (stats?.journeesJouees || 0) * 5) * 100) / 100}€`,
                lbl: 'Gains nets',
                color:
                  (stats?.gainsTotal || 0) - (stats?.journeesJouees || 0) * 5 >= 0
                    ? 'var(--g)'
                    : 'var(--r)',
              },
              { val: stats?.journeesJouees || 0, lbl: 'Journées jouées', color: 'var(--b)' },
              {
                val: stats?.absences || 0,
                lbl: 'Absences',
                color: (stats?.absences || 0) > 0 ? 'var(--r)' : 'var(--tx3)',
              },
            ].map(s => (
              <div
                key={s.lbl}
              ...
              >
                <div
                  style={{
                    ...
                    color: s.color,
                    lineHeight: 1,
                    textShadow: `0 0 14px ${s.color}44`,
                  }}
                >
                  {s.val}
                </div>
                ...
                {paConfig.statut === 'resultats' ? (
                  <div>
                    ...
                    {paConfig.gains?.[user?.uid] > 0 ? (
                      <div style={{ fontSize: 14, color: 'var(--g)', fontWeight: 900 }}>
                        ✅ Tu as gagné +{paConfig.gains[user.uid].toFixed(2)}€ !
                      </div>
                    ) : (
                      <div style={{ fontSize: 13, color: 'var(--tx3)' }}>
                        Pas de gain cette fois — retente l'an prochain.
                      </div>
                    )}
                  </div>
                ) : paMonProno ? (
                  <div>
                    ...
                    <div
                      style={{ fontSize: 11, color: 'var(--tx3)', marginTop: 6, lineHeight: 1.6 }}
                    >
                      Podium : {paMonProno.podium.join(' → ')}
                      <br />
                      LDC : {paMonProno.ldc} · Europa : {paMonProno.europa}
                      <br />
                      ...
                      <>
                        {paConfig.deadline && (
                          <div style={{ fontSize: 10, color: 'var(--tx3)', marginTop: 8 }}>
                            Modifiable jusqu'au{' '}
                            {new Date(paConfig.deadline.seconds * 1000).toLocaleDateString(
                              'fr-FR',
                              {
                                weekday: 'long',
                                day: 'numeric',
                  ...
                  <div>
                    ...
                    >
                      Pronostique le podium final de L1, les vainqueurs LDC/Europa League, et le
                      meilleur buteur/passeur de la saison. Mise totale : 6€.
                    </div>
                    {paConfig.deadline && (
                      <div style={{ textAlign: 'center', marginBottom: 12 }}>
                        ...
                        >
                          ⏰ Deadline :{' '}
                          {new Date(paConfig.deadline.seconds * 1000).toLocaleDateString('fr-FR', {
                            weekday: 'long',
                            day: 'numeric',
                            month: 'long',
                            timeZone: 'Europe/Paris',
                  ...
                  <div style={{ fontSize: 13, color: 'var(--tx3)' }}>
                    Fermé — tu n'as pas soumis de pronostic à temps.
                  </div>
                )}
              </div>
            </>
                  ...
                  >
                    <div
                      style={{
                        ...
                        border: '1px solid rgba(155,226,45,.1)',
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'center',
                        fontSize: 10,
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        <div
                          style={{
                            height: '100%',
                            width: `${(h.pts / maxPts) * 100}%`,
                            background:
                              h.pts >= 5
                                ? 'linear-gradient(90deg,#76B91D,#B9F84F)'
                                : h.pts >= 3
                                  ? 'linear-gradient(90deg,#92400E,#FBBF24)'
                                  : 'linear-gradient(90deg,#7F1D1D,#F87171)',
                            borderRadius: 999,
                            boxShadow: h.pts >= 5 ? '0 0 8px rgba(155,226,45,.3)' : 'none',
                          }}
                        />
                      </div>
                    </div>
              ...
              <div>
                ...
                {pwdMsg && (
                  <div
                    style={{
                      fontSize: 12,
                      color: pwdMsg.startsWith('✅') ? 'var(--g)' : 'var(--r)',
                      marginBottom: 8,
                      fontWeight: 700,
                    }}
                  >
          ...
          {notifStatus !== 'non-supportee' && (
            ...
            >
              ...
              <div style={{ fontSize: 10, color: 'var(--tx3)', marginTop: 6 }}>
                Sur iPhone : ouvre l'app depuis l'icône ajoutée à l'écran d'accueil pour que ça
                fonctionne.
              </div>
            </div>
          )}

          {/* Logo + déco - dans INFOS */}
          {tab === 'infos' && (
              ...
              <picture>
                ...
                <img
                  ...
                  style={{
                    ...
                    margin: '0 auto',
                    opacity: 0.4,
                    filter: 'drop-shadow(0 0 10px rgba(155,226,45,.2))',
                  }}
                />
              </picture>
            </div>
                    ...
                    <div style={{ marginTop: 14 }}>
                      ...
                      {historique.length > 0 && (
                        <div style={{ marginBottom: 16 }}>
                          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8, color: 'var(--tx1)' }}>
                            🏆 Meilleure journée
                          </div>
                          <div
                            style={{
                              background: 'rgba(155,226,45,.05)',
                              border: '1px solid rgba(155,226,45,.15)',
                              borderRadius: 'var(--Rs)',
                              padding: 12,
                              marginBottom: 12,
                              display: 'flex',
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.8)', zIndex: 500 }}>
          <div
            style={{
              ...
              background: 'linear-gradient(180deg, rgba(20,36,27,.99), rgba(9,17,12,.995))',
              border: '1px solid var(--bd)',
              borderBottom: 'none',
              boxShadow: 'var(--shadow)',
            }}
          >
            <div className="page-title" style={{ fontSize: 22, marginBottom: 4 }}>
              🏆 {paMonProno ? 'Modifier mon pronostic' : 'Pronostic saison'}
            </div>
            <div style={{ fontSize: 12, color: 'var(--tx3)', marginBottom: 16 }}>
              Modifiable autant de fois que tu veux jusqu'à la deadline.
            </div>
            ...
            <div className="form-group">
              ...
              <select
                className="input"
                value={paPodium[0]}
                onChange={e => setPaPodium([e.target.value, paPodium[1], paPodium[2]])}
              >
            ...
            <div className="form-group" style={{ marginTop: 10 }}>
              ...
              <select
                className="input"
                value={paPodium[1]}
                onChange={e => setPaPodium([paPodium[0], e.target.value, paPodium[2]])}
              >
                <option value="">— Choisir —</option>
                {CLUBS_L1_2627.map(c => (
                  <option key={c} value={c}>
            ...
            <div className="form-group" style={{ marginTop: 10 }}>
              ...
              <select
                className="input"
                value={paPodium[2]}
                onChange={e => setPaPodium([paPodium[0], paPodium[1], e.target.value])}
              >
                <option value="">— Choisir —</option>
                {CLUBS_L1_2627.map(c => (
                  <option key={c} value={c}>
            ...
            <div className="form-group" style={{ marginTop: 14 }}>
              ...
              <input
                ...
                value={paLdc}
                onChange={e => setPaLdc(e.target.value)}
              />
            </div>
            <div className="form-group" style={{ marginTop: 10 }}>
              ...
              <input
                className="input"
                placeholder="ex: Tottenham"
                value={paEuropa}
                onChange={e => setPaEuropa(e.target.value)}
              />
            </div>
            <div className="form-group" style={{ marginTop: 10 }}>
              ...
              <input
                ...
                value={paButeur}
                onChange={e => setPaButeur(e.target.value)}
              />
            </div>
            <div className="form-group" style={{ marginTop: 10 }}>
              ...
              <input
                className="input"
                placeholder="ex: Ousmane Dembélé"
                value={paPasseur}
                onChange={e => setPaPasseur(e.target.value)}
              />
            </div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              ...
              <button className="btn btn-secondary" onClick={() => setShowParisAnnexe(false)}>
                Annuler
              </button>
            </div>
          </div>
      ...
      {showClubPicker && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.8)', zIndex: 500 }}>
          ...
          >
            ...
            <div style={{ fontSize: 12, color: 'var(--tx3)', marginBottom: 16 }}>
              Ton avatar prend les couleurs du club choisi, visible par tout le monde.
            </div>
            {clubError && (
              <div
                style={{
                ...
                >
                  ...
                  <span
                    style={{
                      fontSize: 12,
                      fontWeight: 700,
                      color: clubCoeur === c ? 'var(--g)' : 'var(--tx2)',
                    }}
                  >
                    {c}
                  </span>
